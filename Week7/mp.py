...
nebula_image = simplegui.load_image("http://commondatastorage.googleapis.com/codeskulptor-assets/lathrop/debris2_blue.png")
...
def draw (canvas):
    ...
    canvas.draw_image(debris_image, center, size, (wtime + WIDTH / 2, HEIGHT / 2), (WIDTH, HEIGHT))
    # draw UI
    canvas.draw_text("Lives:", [20, 30], 20, "Orange")
    canvas.draw_text("Score:", [20, 60], 20, "Orange")
    canvas.draw_text(str(lives), [80, 30], 20, "Orange")
    canvas.draw_text(str(score), [80, 60], 20, "Orange")
...
def rock_spawner():
    ...
    rock_vel = [random.random() * .4 - .2, random.random() * .6 - .3]
